...
class EnvironmentContractTest(unittest.TestCase):
    def test_rejects_legacy_and_production_routes(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            for namespace, route in [('pawbridge', 'environments/dev/isolated-values/animal-service.yaml'), ('pawbridge-dev', 'environments/prod/values/animal-service.yaml')]:
                contract = {'version': 1, 'namespace': namespace, 'branch': 'dev', 'services': {'animal-service': {'devValues': route}}}
    ...
    def test_accepts_only_the_existing_isolated_file(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            (root/'environments/environment-contract.json').write_text(json.dumps({'version': 1, 'namespace': 'pawbridge-dev', 'branch': 'dev', 'services': {'animal-service': {'devValues': relative}}}))
            with self.assertRaises(ValueError): verify(root, 'animal-service')
